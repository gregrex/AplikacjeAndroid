...
function Draw-Mark {
    param([System.Drawing.Graphics]$Graphics, [float]$X, [float]$Y, [float]$Scale)
...
function Save-FeatureGraphic {
    param([string]$Locale, [string]$ProductName, [string]$Headline, [string]$Subline)

    $fileName = if ($Locale -eq "default") { "feature-graphic-1024x500.png" } else { "feature-graphic-$Locale-1024x500.png" }
    ...
    $productFont = [System.Drawing.Font]::new("Arial", 42, [System.Drawing.FontStyle]::Bold, [System.Drawing.GraphicsUnit]::Pixel)
    ...
    $blueBrush = [System.Drawing.SolidBrush]::new([System.Drawing.ColorTranslator]::FromHtml("#38BDF8"))
    $bodyBrush = [System.Drawing.SolidBrush]::new([System.Drawing.ColorTranslator]::FromHtml("#CBD5E1"))
    $smallBrush = [System.Drawing.SolidBrush]::new([System.Drawing.ColorTranslator]::FromHtml("#94A3B8"))

    $graphics.DrawString("AppFactory", $titleFont, [System.Drawing.Brushes]::White, 92, 142)
    $graphics.DrawString($ProductName, $productFont, $blueBrush, 92, 210)
    $graphics.DrawString($Headline, $bodyFont, $bodyBrush, 92, 282)
    $graphics.DrawString($Subline, $smallFont, $smallBrush, 92, 334)
    ...
    $blueBrush.Dispose(); $bodyBrush.Dispose(); $smallBrush.Dispose()
...
$icon = Save-Icon
$features = @(
    Save-FeatureGraphic "default" "Pomocniki" "20 praktycznych narzędzi w jednej aplikacji" "Dom • styl • hobby • codzienne diagnozy"
    Save-FeatureGraphic "pl-PL" "Pomocniki" "20 praktycznych narzędzi w jednej aplikacji" "Dom • styl • hobby • codzienne diagnozy"
    Save-FeatureGraphic "en-US" "Helpers" "20 practical tools in one app" "Home • style • hobbies • everyday guidance"
    Save-FeatureGraphic "uk-UA" "Помічники" "20 практичних інструментів в одному застосунку" "Дім • стиль • хобі • щоденні поради"
)
...
Write-Host "- $icon"
foreach ($feature in $features) { Write-Host "- $feature" }
